...
from .proactive_orchestration_engine import ProactiveOrchestrationEngine, integrate_with_roo_commander
from .continuous_learning_system import ContinuousLearningSystem, integrate_continuous_learning
from .convex_orchestrator import ConvexOrchestrator, integrate_convex_system
from ..rag.engines.agentic_rag import AgenticRAGEngine, integrate_agentic_rag

# Learning Insights Thresholds
CLS_COMPLETION_RATE_THRESHOLD = 0.8
CLS_ERROR_FREQUENCY_THRESHOLD = 0.1
CLS_USER_RATING_THRESHOLD = 3.5

# Overall Confidence Calculation Parameters
CONF_RAG_WEIGHT = 0.6
CONF_POE_WEIGHT = 0.3
CONF_SYSTEM_HEALTH_BASE = 0.8 # Default confidence if no other factors
CONF_SYSTEM_HEALTH_WEIGHT = 0.1 # Weight if other factors are present
CONFIDENCE_VALUE_MAP = {'high': 1.0, 'medium': 0.7, 'low': 0.4, 'insufficient': 0.1, 'default': 0.5}
...
class UltimateAgenticResponse:
    ...
    convex_insights: Dict[str, Any]  # CONVEX dual-brain insights
    learning_feedback: Dict[str, Any]
    confidence_score: float
    execution_time: float
    system_improvements: List[str]
    routing_decision: str  # How CONVEX routed the query
...
class UltimateAgenticOrchestrator:
    ...
    def __init__(self, conport_client, vertex_ai_client=None):
        ...
        self.convex = ConvexOrchestrator(conport_client, vertex_ai_client)  # CONVEX dual-brain system
    ...
    async def process_ultimate_request(self, user_query: str, workspace_id: str, 
        ...
        try:
            ...
            convex_task = self._execute_convex_system(user_query, workspace_id)  # CONVEX dual-brain
            learning_task = self._execute_continuous_learning(workspace_id)
            
            # Execute in parallel for maximum efficiency
            rag_response, poe_suggestions, convex_response, learning_feedback = await asyncio.gather(
                rag_task, poe_task, convex_task, learning_task, return_exceptions=True
            )
            ...
            return UltimateAgenticResponse(
                ...
                convex_insights=self._extract_convex_insights(convex_response),
                learning_feedback=learning_insights,
                confidence_score=confidence_score,
                execution_time=execution_time,
routing_decision=self._determine_routing_decision(self._extract_convex_insights(convex_response)),
    ...
    async def _execute_convex_system(self, query: str, workspace_id: str):
        """Execute CONVEX dual-brain system with error handling"""
        try:
            return await self.convex.process_convex_query(query, workspace_id)
        except Exception as e:
            self.logger.error(f"CONVEX execution failed: {e}")
            return None
    ...
    def _extract_learning_insights(self, learning_feedback: Dict) -> List[str]:
        ...
        for agent_data in agents_analyzed:
            ...
            if completion_rate < CLS_COMPLETION_RATE_THRESHOLD:
                insights.append(f"{agent}: Low completion rate ({completion_rate:.1%})")
            
            if error_frequency > CLS_ERROR_FREQUENCY_THRESHOLD:
                insights.append(f"{agent}: High error rate ({error_frequency:.1%})")
            
            if user_rating < CLS_USER_RATING_THRESHOLD:
    ...
    def _calculate_overall_confidence(self, rag_response, poe_suggestions) -> float:
        ...
        if rag_response and hasattr(rag_response, 'confidence_level'):
            rag_confidence = CONFIDENCE_VALUE_MAP.get(rag_response.confidence_level.value, CONFIDENCE_VALUE_MAP['default'])
            confidence_factors.append(rag_confidence * CONF_RAG_WEIGHT)
        
        # POE confidence (based on suggestion quality)
        if poe_suggestions and isinstance(poe_suggestions, list) and len(poe_suggestions) > 0: # check if list is not empty
            avg_poe_confidence = sum(getattr(s, 'confidence_score', 0.5) for s in poe_suggestions) / len(poe_suggestions)
            confidence_factors.append(avg_poe_confidence * CONF_POE_WEIGHT)
        
        # System health (simplified)
        confidence_factors.append(CONF_SYSTEM_HEALTH_BASE * CONF_SYSTEM_HEALTH_WEIGHT)
    ...
    def _extract_convex_insights(self, convex_response) -> Dict[str, Any]:
        """Extract insights from CONVEX response"""
        if not convex_response:
            return {'status': 'no_convex_response'}
        
        return {
            'status': 'active',
            'convex_routing': getattr(convex_response, 'routing_decision', 'unknown'),
            'dual_brain_analysis': getattr(convex_response, 'dual_brain_results', {}),
            'convex_confidence': getattr(convex_response, 'confidence_score', 0.5)
        }
    
    def _determine_routing_decision(self, convex_insights) -> str:
        """Determine how CONVEX routed the query"""
        return convex_insights.get('convex_routing', 'standard_processing') if convex_insights else 'no_convex'
...
routing_decision='error_fallback',
    ...
    async def run_system_optimization_cycle(self, workspace_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'poe_suggestions_count': len(poe_suggestions) if poe_suggestions else 0, # Added check
    ...
    def _calculate_performance_trend(self, metrics: List[Dict]) -> str:
        ...
        # Ensure metrics list is not empty before division
        half_len = len(metrics) // 2
        if half_len == 0: # handles cases where len(metrics) is 1, 2, 3, or 4
            return 'insufficient_data'

        early_avg = sum(m['confidence_score'] for m in metrics[:half_len]) / half_len
        late_avg = sum(m['confidence_score'] for m in metrics[half_len:]) / (len(metrics) - half_len)
    ...
    def _generate_optimization_recommendations(self, learning_results: Dict, 
        ...
        if learning_results and learning_results.get('proposals_generated'): # Added check for learning_results
            recommendations.append("Review and implement agent improvement proposals")
        
        # POE-based recommendations
        if poe_suggestions is None or len(poe_suggestions) < 3: # Added check for poe_suggestions being None
...
# Main integration function for Roo Commander
